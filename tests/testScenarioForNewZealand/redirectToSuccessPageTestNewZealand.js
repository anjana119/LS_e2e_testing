//  To test this content ->  npm tag test : npm test -- --tag redirectToSuccessPageTestNewZealand

const DefaultConfig = require('../DefaultConfig.json');

module.exports = {
    '@tags': ['redirectToSuccessPageTestNewZealand'],
    'Luxury Escapes Redirect To Success Page Test NewZealand'(browser) {
        ...
        const confirmTravelerDetailsButton = 'button[data-testid="confirm-traveller-details"]';

        const acceptAndReturnToCheckout = '.cVhTij';

        // ------------ identify confirmation page details -------------
        const purchaseConfirmedSection = 'div[data-testid="purchase-confirmed"]';
        const airlineBookingReferenceNoSection = 'div[data-testid="flight-item"]';
        ;
        browser
        ...
        browser.element('css selector', selectFlightCategory, function (result) {
            if (result.status === -1) {
                ...
                // ------------ click confirm traveler details ------------
                browser
                    .waitForElementPresent(confirmTravelerDetailsButton, 500000, 'Confirm traveller details model present !')
                    .click(confirmTravelerDetailsButton)
                    .pause(5000);

                browser.waitForElementPresent({ selector: acceptAndReturnToCheckout, index: 26 }, 100000, 'Accept new price & return to Checkout model is present !')
                browser.element('css selector', { selector: acceptAndReturnToCheckout, index: 26 }, function (result) {
                    console.dir(result);
                    if (result.status === -1) {
                        browser
                            .click({ selector: acceptAndReturnToCheckout, index: 26 })
                            .pause(9000);
                        browser
                            .waitForElementPresent(paySecurelyNowButton, 50000)
                            .click(paySecurelyNowButton);
                    }
                });

                browser
                    .waitForElementPresent(purchaseConfirmedSection, 500000, 'Booking confirmed page present !')
                    .refresh();

                browser.waitForElementPresent(airlineBookingReferenceNoSection, 10000, 'Airline Booking Reference No is present !');

                browser.getText(airlineBookingReferenceNoSection, (result) => {
        ...
        browser.saveScreenshot('tests_output/screenshots/newZealand/PaySecurelyTestNewZealand.png');
    }
};